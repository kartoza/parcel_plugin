...
class controller():
    ...
    def editPoint(self):
        """ Edit existing point
        """
        dlg = ui_editor(self.pointDict, self.iface, self.layers["points"])
        dlg.show()
        dlg.exec_()
...
class ui_form(QDialog):
    ...
    # checked
    def __init__(self, pointDict, point_id = None):
        # initialize dialog
        ...
        if self.point_id is not None:
            values = [v[0] for v in self.dbmanager.query(self.pointDict["sql"]["edit"].format(fields = ", ".join([d["name"] for d in self.pointFields]), object_id = self.point_id))]
...
class ui_editor(QDialog):
    ...
    def __init__(self, pointDict, iface, layer):
        ...
        self.ui.setupUi(self, pointDict["name"], "Editor", "edit")
...
class ui_remover(QDialog):
    ...
    def __init__(self, pointDict, iface, layer):
        ...
        QDialog.__init__(self, None, Qt.WindowStaysOnTopHint)
        self.ui = ui_dlg_objectSelector()
        self.ui.setupUi(self, pointDict["name"], "Remover", "delete")
        # save additional qgis references
        self.pointDict = pointDict
        self.iface = iface
        self.layer = layer
